/**
 * This class is used to create couples
 * @author Raghav Tayal
 *
 */
public class Couple {
	...
	double hap;
	int n;
	Couple(Boy boy, Girl girl, Gift gift[], int no){//Class constructor
		this.boy = boy;
		this.girl = girl;
		this.gt = gift;
		this.hap = 0;
	 ...
	 void Happiness()
	{
		...
		/**
		 * Happiness for particular girl type using sitch statement
		 */
			switch(girl.type) {
			case "Choosy":
				hap = (hap + Math.log(sum +value+val_ch - girl.maint_budg))%30;
				if(hap < 0 ) {
					hap = 0;
				}
				break;
			case "Normal":
				hap = (hap +sum + value - girl.maint_budg)%30;
				break;
			case "Desperate":
